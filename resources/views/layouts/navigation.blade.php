<nav x-data="{ open: false }" class="bg-white border-b ">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    ...
                    <x-nav-link :href="route('admin.properties.index')" :active="request()->routeIs('admin.properties.*')" class="text-gray-300">
                        {{ __('Propiedades') }}
                    </x-nav-link>
                </div>
            </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-black-300 focus:outline-none transition ease-in-out duration-150">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-slate-800">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('admin.properties.index')" :active="request()->routeIs('admin.properties.*')" class="text-gray-300 hover:text-white hover:bg-slate-700">
                {{ __('Propiedades') }}
            </x-responsive-nav-link>
        </div>
            ...
            <div class="mt-3 space-y-1">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    ...
                    <x-responsive-nav-link :href="route('logout')"
                            onclick="event.preventDefault();
                                        this.closest('form').submit();" class="text-gray-300 hover:text-white hover:bg-slate-700">
                        {{ __('Cerrar Sesión') }}
                    </x-responsive-nav-link>
                </form>
            </div>
